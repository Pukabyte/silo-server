...
func TestRTBackfillPersistsScores(t *testing.T) {
	...
	b := newRTBackfiller(fetcher, updater)
	if b == nil {
		t.Fatal("newRTBackfiller returned nil for valid deps")
	}
...
func TestRTBackfillNegativeCacheOnNoRT(t *testing.T) {
	...
	b := newRTBackfiller(fetcher, &stubRTUpdater{})
...
func TestRTBackfillDedupeInFlight(t *testing.T) {
	b := newRTBackfiller(&stubRTFetcher{}, &stubRTUpdater{})
	if !b.claim("ct3") {
...
func TestRTBackfillNilSafety(t *testing.T) {
	if newRTBackfiller(nil, &stubRTUpdater{}) != nil {
